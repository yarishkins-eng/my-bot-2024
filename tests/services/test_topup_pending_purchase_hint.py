...
@pytest.mark.anyio
async def test_the_lookup_asks_about_every_subscription_of_this_person() -> None:
    """Сторож на САМ ЗАПРОС: фикстуры выше подсовывают готовые строки и заборов не проверяют.

    🔴 ИМЯ И СМЫСЛ ЗДЕСЬ ВАЖНЕЕ ОБЫЧНОГО. Первая редакция называлась «...only about live
    subscriptions» и заканчивалась комментарием «истёкшая и отключённая живыми не считаются» —
    а тело утверждало ровно обратное. Ревизия сказала прямо: следующий читатель «починит» такой
    сторож обратно и переоткроет мину HX. Запрос НЕ фильтрует по статусу НАМЕРЕННО: забор по
    автоплатежу живёт внутри него, и фильтр прятал бы истёкшую платную подписку с автоплатежом —
    ту самую, по которой работает автопродление.
    ...
    # 🔴 Мина HX. Стережём ОТСУТСТВИЕ фильтра по статусу. Ни один статус в запрос попасть не
    # должен: появился — значит кто-то вернул фильтр и снова спрятал от забора автоплатежа
    # истёкшую платную подписку, по которой работает автопродление.

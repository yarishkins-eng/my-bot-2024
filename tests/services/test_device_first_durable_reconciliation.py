...
def test_no_way_to_move_a_client_between_servers_exists_today():
    ...
    🔴 19.08 (пункт 3.2) утверждение ПЕРЕВЕРНУЛОСЬ. Раньше текст говорил «готовой кнопки
    нет», и сторож следил, чтобы не ожил ни один из трёх мёртвых путей. Кнопка появилась —
    но не там, где её ждали: не «Переезд сквада», а раскатка серверов тарифа. Сторож этого
    не заметил, потому что смотрел на конкретные три модуля, а не на смысл утверждения.
    Теперь он сторожит обе половины текста: что названная кнопка ЖИВА и что опасные пути
    по-прежнему мертвы.
    """
    import inspect
    ...
    # 🔴 Вторая половина: текст ТЕПЕРЬ называет владельцу конкретную кнопку. Если её
    # маршрут исчезнет, тревога снова начнёт врать — уже в обратную сторону.
    from app.cabinet.routes import admin_tariffs

    rollout_paths = {route.path for route in admin_tariffs.router.routes}
    assert '/admin/tariffs/{tariff_id}/squad-rollout' in rollout_paths, (
        'маршрут раскатки исчез — тревога зовёт владельца в кнопку, которой больше нет'
    )

    from app.services import device_first_checkout_service as checkout_service

    alert_source = inspect.getsource(checkout_service._send_owner_checkout_drift_alert)
    assert 'Раскатать серверы на подписки' in alert_source, 'название кнопки в тревоге разошлось с кабинетом'
    assert 'Готовой кнопки' not in alert_source, 'вернулось утверждение «кнопки нет», хотя она есть'
    # 🔴 Запрет держит защиту, а не украшает текст: «Сбросить подписку» физически удаляет
    # подписку вместе с оплаченным сроком. Мутация показала, что его можно было снять молча.
    assert 'Не пытайтесь' in alert_source and 'Сбросить подписку' in alert_source, (
        'из тревоги исчезло предупреждение про «Сбросить подписку» — оно защищает оплаченный срок'
    )

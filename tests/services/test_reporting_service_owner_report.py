...
def _seed_owner_day(seed: _Seed) -> None:
    ...
    # бьёт — реферальные начисления пишутся типом `referral_reward`, не пополнением: на боевом все 68 проводок
    # с `description ILIKE '%еферал%'` — `referral_reward`, пополнений среди них 0 (замер 24.09.2026)

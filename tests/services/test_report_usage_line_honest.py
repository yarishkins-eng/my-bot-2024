...
async def test_usage_query_measures_servers_and_not_connections() -> None:
    ...
    assert 'distinct' in sql, 'запрос считает подписки, а строка обещает пользователей'
    # Надпись говорит «за всю историю» — значит фильтра по статусу и сроку быть не должно.
    for narrowing in ('status', 'end_date'):
        assert narrowing not in sql, f'запрос сузился по {narrowing} — «за всю историю» в строке отчёта стало ложью'

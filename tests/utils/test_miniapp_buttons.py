...
def test_build_cabinet_url_keeps_a_version_query_after_deep_link(monkeypatch) -> None:
    monkeypatch.setattr(
        settings,
        'MINIAPP_CUSTOM_URL',
        'https://cabinet.example.com:8443?release=cabinet-6020269',
        raising=False,
    )

    assert build_cabinet_url('/') == 'https://cabinet.example.com:8443?release=cabinet-6020269'
    assert build_cabinet_url('/balance') == 'https://cabinet.example.com:8443/balance?release=cabinet-6020269'
    assert build_cabinet_url('connection') == 'https://cabinet.example.com:8443/connection?release=cabinet-6020269'
...
def test_build_cabinet_url_keeps_a_base_path_and_fragment(monkeypatch) -> None:
    monkeypatch.setattr(
        settings,
        'MINIAPP_CUSTOM_URL',
        'https://cabinet.example.com/app/?release=cabinet-6020269#telegram',
        raising=False,
    )

    assert build_cabinet_url('/') == 'https://cabinet.example.com/app?release=cabinet-6020269#telegram'
    assert (
        build_cabinet_url('/connection')
        == 'https://cabinet.example.com/app/connection?release=cabinet-6020269#telegram'
    )
...
def test_campaign_link_keeps_the_version_query(monkeypatch) -> None:
    ...
    assert (
        ...
        == 'https://cabinet.example.com:8443?release=cabinet-6020269&campaign=summer+sale'
    )

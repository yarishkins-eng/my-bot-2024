...
[ "$RECOVERY_PHASE" = 'prepared' ] || [ "$RECOVERY_PHASE" = 'completed' ] || [ "$RECOVERY_PHASE" = 'recovered' ] || fail 'invalid_recovery_phase'
...
if [ "$RECOVERY_PHASE" = 'recovered' ]; then
  [ "$DEPLOY_CLASS" = 'recovery' ] || fail 'recovered_without_recovery_state'
fi

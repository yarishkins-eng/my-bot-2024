...
class Tariff(Base):
    ...
    # Public-location entitlement is deliberately additive.  The legacy UUID
    # array remains untouched until an owner-approved manifest cutover; no
...
class Subscription(Base):
    ...
    # Immutable, business-level entitlement evidence.  `connected_squads`
    # remains the panel projection only and must never be reconstructed from
    # a mutable tariff policy after payment has been captured.
    entitlement_snapshot = Column(JSON, nullable=True)
    entitlement_provenance = Column(String(32), nullable=True)
    entitlement_policy_revision = Column(Integer, nullable=True)
...
# ---------------------------------------------------------------------------
# Public location entitlements
#
# These records model the product contract, not RemnaWave implementation
# details.  Technical squad UUIDs are intentionally kept server-side in the
# mapping table and are never serialised by public/user-facing APIs.
# ---------------------------------------------------------------------------
...
class PublicLocation(Base):
    ...
    label_ru = Column(String(120), nullable=False)
    label_en = Column(String(120), nullable=False)
    flag = Column(String(16), nullable=True)
    sort_order = Column(Integer, nullable=False, default=0)
    lifecycle = Column(String(16), nullable=False, default='draft')
    visibility = Column(String(16), nullable=False, default='hidden')
    health = Column(String(16), nullable=False, default='unknown')
...
class PublicLocationSquadMapping(Base):
    ...
    internal_squad_uuid = Column(String(255), nullable=False, unique=True)
    is_dedicated_verified = Column(Boolean, nullable=False, default=False)
    inventory_revision = Column(String(128), nullable=True)
    inventory_membership_hash = Column(String(128), nullable=True)
    created_at = Column(AwareDateTime(), nullable=False, default=func.now())
...
class TariffLegacyEntitlementManifest(Base):
    """Approved legacy UUID snapshot; never generated by an Alembic revision."""

    __tablename__ = 'tariff_legacy_entitlement_manifests'
...
class SubscriptionEntitlementSnapshot(Base):
    ...
    id = Column(Integer, primary_key=True)
    subscription_id = Column(Integer, ForeignKey('subscriptions.id', ondelete='RESTRICT'), nullable=False, unique=True)
    tariff_id = Column(Integer, ForeignKey('tariffs.id', ondelete='RESTRICT'), nullable=True)
    location_ids = Column(JSON, nullable=False)
...
class EntitlementChangePlan(Base):
    """Durable preview/approval state only; this release has no live executor."""

    __tablename__ = 'entitlement_change_plans'

    id = Column(String(36), primary_key=True)
    state = Column(String(32), nullable=False, default='previewed')
    ...
    plan_hash = Column(String(128), nullable=False, unique=True)
    policy_revision = Column(Integer, nullable=False)
    scope = Column(JSON, nullable=False)
    preimage = Column(JSON, nullable=False)
    created_at = Column(AwareDateTime(), nullable=False, default=func.now())
    confirmed_at = Column(AwareDateTime(), nullable=True)


class EntitlementPlanApproval(Base):
    __tablename__ = 'entitlement_plan_approvals'

    id = Column(Integer, primary_key=True)
    plan_id = Column(String(36), ForeignKey('entitlement_change_plans.id', ondelete='RESTRICT'), nullable=False)
    actor_user_id = Column(Integer, ForeignKey('users.id', ondelete='RESTRICT'), nullable=False)
...
class EntitlementPlanJob(Base):
    ...
    target_squads = Column(JSON, nullable=False)
    preimage = Column(JSON, nullable=False)
    error = Column(Text, nullable=True)
    created_at = Column(AwareDateTime(), nullable=False, default=func.now())
    updated_at = Column(AwareDateTime(), nullable=False, default=func.now(), onupdate=func.now())
...
class EntitlementPlanOutbox(Base):
...
class EntitlementPlanCheckpoint(Base):
    __tablename__ = 'entitlement_plan_checkpoints'

    id = Column(Integer, primary_key=True)
    job_id = Column(String(36), ForeignKey('entitlement_plan_jobs.id', ondelete='RESTRICT'), nullable=False, index=True)

...
# 🔴 РЕК-8в. Единственное «остановившееся» состояние, где деньги ВЗЯТЫ и которое умеет
# показать `get_open_checkout_for_user`. Соседние три (`conflict`, `failed`,
# `reprice_required`) — доплатёжные и этой функцией не выбираются; подробности в докстроке
# `cancel_fused`.
OPERATOR_REVIEW_STATE = 'operator_review'
...
async def _has_order_in_flight(db: AsyncSession, *, user_id: int, states: Collection[str] = OPEN_STATES) -> bool:
    ...
    helper expires a stale quote and ``db.commit()``s it, and it calls
    ...
    молча поменять поведение экрана заказа, у которого своя причина и свой сторож. А вопрос
...
def _money_block(
    ...
    *,
    balance_kopeks: int,
    price_kopeks: int,
    top_up_button: InlineKeyboardButton | None,
    has_methods: bool,
) -> tuple[str, str]:
    """Build the money paragraph both «Ваш заказ» screens print under the price.

    🔴 One builder on purpose. The bot draws this order twice — before the order
    row exists and after — and until now the two screens agreed only by accident.
    Two editions of one money fact are how this product got its reputation.
    """
    has_full = balance_kopeks >= price_kopeks
    if has_full:
        return ('Оплатите с баланса.', 'Pay from your balance.')
    if balance_kopeks <= 0:
        return ('Выберите способ оплаты.', 'Choose a payment method.')
    # Слова взяты у самого бота — ими он уже называет эти два факта на экране подтверждения
    # заказа (`_render_arm_confirmation`). Недостача — честная разность, без провайдерских
    # минимумов: её же печатает и кабинет.
    shortage = price_kopeks - balance_kopeks
    wallet_ru = f'💳 Баланс: {_money(user, balance_kopeks)} ₽\n⚠️ Не хватает: {_money(user, shortage)} ₽\n\n'
    wallet_en = f'💳 Balance: ₽{_money(user, balance_kopeks)}\n⚠️ Shortage: ₽{_money(user, shortage)}\n\n'
    if not has_methods:
        # Способов оплаты нет — ниже экран сам скажет «Оплата временно недоступна». Звать
        # выбрать способ и рассуждать про списание, которого не может быть, значит спорить
        # с собой на глазах у человека, чьи деньги мы строкой выше назвали.
        return (wallet_ru.rstrip(), wallet_en.rstrip())
    if top_up_button is None:
        # Урок мины DE: строка про баланс рядом с ценой читается как «зачтётся», а он не
        # зачитывается — человек платит полную цену, а деньги остаются лежать. Оговорка
        # обязана стоять там же, где строка, иначе строка врёт.
        return (
    ...
    # 🔴 И вторая строка обязательна: «вернитесь» без указания КУДА посылает человека ровно в
    # этот чат — а здесь его ждёт ЭТО ЖЕ сообщение с числами, которые доплата только что сделала
    # ложными, и с кнопками полной цены. Перерисовать его нечем: дверь и кнопки оплаты — web_app,
    # боту при нажатии не приходит ничего, а деньги зачисляет вебхук.
    ...
    hint_ru = (
        'Ваш баланс уже учтён в строке «Не хватает». '
        'Доплатите и продолжите покупку в том же окне — ваш выбор сохранится.'
    )
    hint_en = (
        'Your balance is already counted in the «Shortage» line. '
        'Top up and finish the purchase in the same window — we keep your choice.'
    )
    ...
    # Порядок именно такой: в Телеграме подпись всегда ВЫШЕ кнопок, поэтому доплата называется
    # первой, а «или» встаёт после того, чему противопоставлено.
    return (
...
async def _render_direct_payment_methods(
    ...
    price = checkout.tariff_total_kopeks
    has_full_wallet_balance = user.balance_kopeks >= price
    has_partial_wallet = 0 < user.balance_kopeks < price
    # 🔴 Забор здесь ДРУГОЙ, чем на первом экране, и это не небрежность. Там вопрос был
    # «есть ли у человека заказ в работе»; здесь заказ есть по определению — этот экран
    # его и рисует. Спрашивать надо другое: закреплён ли за заказом способ оплаты. Закреплён
    # (счёт выставлен ``prepare_direct_external_checkout`` или списание подготовлено
    # ``commit_direct_wallet_checkout``) — доплата кончится отказом ``funding_mode_locked``,
    # а человек, только что положивший ровно недостающую сумму, упрётся в тупик.
    # ⛔ НЕ УБИРАТЬ КАК «ДОКАЗАННО МЁРТВЫЙ». Прежняя редакция комментария утверждала, что все
    # пять мест, зовущих этот экран, срабатывают до создания счёта. Это неверно для пятого:
    # обработчик ``arm`` (``df:a:``) пускает сюда любое состояние из ``OPEN_STATES``, а туда
    # входят ``awaiting_funds`` и ``fulfilling`` — те самые, в которые заказ переходит ВМЕСТЕ
    # с закреплением способа оплаты, одной транзакцией. Четыре места действительно приходят
    # раньше счёта, пятое — нет, и забор там единственная защита.
    # ⚠️ И он одноразовый по своей природе: считается в момент отрисовки, а кнопки на уже
    # отправленном сообщении живут дальше. Это мина FE, она открыта и покрывает ОБА экрана.
    top_up_button = (
        _top_up_button(
            user,
            days=checkout.period_days,
            devices=checkout.selected_device_limit,
            price=price,
        )
...
async def _render_fused_confirmation(
    ...
    *,
    ...
    # Порядок важен: сперва бесплатные признаки, и только потом запрос к базе. У человека
    # с нулём, у того, кому хватает, и у того, чья доплата равна полной цене, лишнего запроса
    # на пути отрисовки не случается вовсе.
    top_up_button = (
...
async def cancel_fused(
    ...
    """Discard the showcase — and never claim to cancel an order that outlived it.

    🔴 РЕК-8в. Прежняя редакция выбрасывала соединение с базой (``del db``) и БЕЗУСЛОВНО писала
    «Заказ отменён. Деньги не списаны», ни разу не заглянув в базу. В момент ОТРИСОВКИ экрана
    заказа действительно нет — но соседние кнопки на нём открывают кабинет меткой
    ``autostart=1``, и заказ рождается ТАМ, пока сообщение в чате остаётся прежним. Человек,
    ...
    🔴 Спрашиваем ШИРЕ открытых состояний — но ровно на ОДНО состояние, а не на четыре.
    Волна 1 добавила весь ``OPERATOR_REVIEWABLE_STATES``, волна 2 это опровергла, и обе
    поправки записаны здесь, чтобы следующий не «дорасширил» обратно:
      · ``operator_review`` — деньги ВЗЯТЫ, подписки нет. Он и нужен, ради него правка;
      · ``conflict`` / ``reprice_required`` — ДОплатёжные: их ставят
        ``expire_checkout_quote_if_needed`` («expire an open quote before any money-moving
        transition») и предкоммитная сверка. Сам сервис пишет о них прямым текстом:
        «остановившийся заказ покупку не держит вовсе»;
      · ``failed`` — сегодня не присваивается нигде.
    ⛔ И решающее: эти три состояния ``get_open_checkout_for_user`` НЕ выбирает, то есть кнопка
    ниже их показать не может — мы обещали бы «откройте заказ» и приводили на выбор срока.
    ⛔ Хуже: ``reprice_required`` ставится обычным истечением котировки и не снимается никем.
    Спрашивая его, мы сказали бы «у вас открыт заказ» КАЖДОМУ, кто когда-либо бросил расчёт, —
    и поменяли бы одну неправду на другую, в обратную сторону.
        ...
        await edit_or_answer_photo(
            ...
            keyboard=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text=_text(db_user, 'К моему заказу', 'My order'),
                            callback_data='df:start',
                        )
                    ],
                    [_main_menu(db_user)],

...
async def _show_trial_checkout_resolution(
    ...
    if context.state == 'pending_invoice' and context.checkout is not None:
        ...
        # Пункт 4.2б: «предыдущую оплату» утверждает платёж, которого могло не быть — сюда
        # приходит и брошенная корзина. Близнец этого экрана в кабинете
        # (`TrialStart`, ветка `isChecking`) поправлен тем же пунктом, тексты держим парой.
        message = (
            '⏳ <b>Проверяем предыдущий заказ</b>\n\n'
            'Чтобы не создать лишний счёт, откройте кабинет и обновите статус заказа.'
        )
    ...
    if user_balance_kopeks >= price_kopeks:
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                # этапом 5 «вынести мусор» (записано отдельной строкой в плане, а не только тут:
                # комментарии протухают молча — мина AB). Глушим ровно ОДНУ строку, чтобы ворота

...
async def change_subscription_type(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if subscription.is_trial:
        # 🔴 Мина AA: кнопку «Сделать платной» не рисуем — она создавала «фантомную платную»
        # с триальным сроком и снимала сторожа автосписания (подробно в
        # `change_subscription_type_confirm`, там же стоит и защита от старой кнопки в чате).
        # Вместо неё — куда идти за настоящей платной подпиской.
        text += (
            '\n\n⛔ «Сделать платной» отключено: подписка получила бы платную метку при '
            'триальном сроке. После этого клиенту открывается автоплатёж (триалу он запрещён), '
            'и ближайшее пополнение баланса может списать деньги за полный период.\n'
            '💡 Настоящую подписку выдайте в кабинете: «Сбросить подписку» → «Создать '
            'подписку» → выбрать тариф и срок.'
        )
    else:
        # ⚠️ Тумблер стал односторонним: обратной кнопки «Сделать платной» больше нет (мина AA),
        # и вернуть платный тип нечем — ни здесь, ни в кабинете, ни в веб-админке. На живой
        # годовой подписке один случайный клик подменит клиенту меню на триальное и отключит
        # автопродление, а починка будет стоить сброса подписки вместе с её сроком и профилем.
        text += (
            '\n\n⚠️ Обратно платной эту подписку сделать будет нечем: кнопка «Сделать платной» '
            'отключена. Возврат — только «Сбросить подписку» и «Создать подписку» заново в кабинете.'
        )
        keyboard.append(
...
async def change_subscription_type_confirm(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    # 🔴 Мина AA. Направление «в платную» отключено: оно снимало ОБА сторожа автосписания
    # бага #629889 одним нажатием. `_change_subscription_type` ставит `is_trial=False`
    ...
    # `webapi/routes/miniapp.py:4980`) и разрешает «платной».
    # 🟢 Обратное направление живое и безопасное: оно взводит гард обратно.
    # 🟢 Владелец ничего не теряет: полное меню подписчика даёт кабинет — «Сбросить подписку»,
    # затем «Создать подписку» с выбором тарифа. Там свежая `is_trial=False` с нормальным
    # сроком и настоящими серверами, и `has_had_paid_subscription` не портится.
    # ⚠️ Снять флаг обратно из чат-админки нельзя: «🎁 Сделать триальной» возвращает `is_trial`,
    # но `has_had_paid_subscription` остаётся навсегда.
    # 🔴 Сравнение с 'trial', а не с 'paid': сама мутация считает платным ВСЁ, что не `trial`
    # (`new_is_trial = new_type == 'trial'`), поэтому чёрный список из одного слова пропустил бы
    # любой другой токен в те же присваивания. Гард обязан совпадать по смыслу с тем, что защищает.
    # ⚠️ Текст обязан уместиться в 200 символов: столько принимает `answerCallbackQuery`, а длиннее
    # Telegram отвечает 400, и человек видит общее «Произошла ошибка» вместо объяснения — то есть
    # ровно в старых сообщениях с этой кнопкой, где другого текста уже не будет.
    if new_type != 'trial':
        await callback.answer(
            'Отключено: подписка станет «платной» с триальным сроком. Выдайте настоящую в кабинете: '
            '«Сбросить подписку» → «Создать подписку» с тарифом.',
            show_alert=True,
        )
        return

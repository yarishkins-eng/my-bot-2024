...
from app.handlers.admin import (
    ...
    orders_review as admin_orders_review,
...
def _register_update_handlers(dp: Dispatcher) -> None:
    """Register every update handler, ending with the unknown-callback router."""
    # Командные хендлеры меню ☰ регистрируем ПЕРВЫМИ: иначе FSM-обработчики
    # ввода (промокод/вывод средств/тикеты) перехватывают команду как «ввод».
    ...
    admin_orders_review.register_handlers(dp)
    ...
    # Must be the terminal child router: a root catch-all would shadow every child router.
    common.register_unknown_callback_fallback(dp)

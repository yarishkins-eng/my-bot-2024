...
async def count_trial_and_paying_users(db: AsyncSession) -> dict[str, int]:
    ...
    Одно определение на два экрана (ОТЧ-7): плитки кабинета «На пробном» / «С платной подпиской» и утреннее
    письмо владельцу. Переехало из `app/cabinet/routes/admin_users.py` без изменений логики (ТИМ-3).
    Правила владельца: пробный — не подписка; платит — только кто платил деньгами; Team — не клиенты;
    стенды и удалённые — не люди.
    """

...
async def topup_pending_purchase_hint(user: Any) -> str | None:
    """Назвать оставшийся шаг тому, кому он нужен — и промолчать всем остальным.
    ...
    🔴 МОЛЧИМ ВСЕГДА, КОГДА ЭТИ ЖЕ ДЕНЬГИ СОБИРАЕТСЯ ПОТРАТИТЬ КТО-ТО ДРУГОЙ. Сразу после нашего
    сообщения вебхук зовёт ``send_cart_notification_after_topup``, а тот — автопокупку корзины и
    автопродление истёкшей. Оба списывают баланс сами и шлют своё «подписка оформлена». Сказать
    перед ними «нажмите кнопку» значит не просто соврать через секунду, а подтолкнуть человека
    купить ВТОРОЙ период поверх оплаченного. Поэтому два забора:
    · есть свежая метка «пополняю ради корзины» — автопокупка и потратит деньги, и объяснится
      сама; больше того, на экране нехватки бот ЗАРАНЕЕ пообещал ей «корзина сохранена, после
      пополнения подписка будет оформлена автоматически» (`tariff_purchase.py:975`), и наша
      фраза спорила бы сразу с двумя его же сообщениями — прошлым и следующим;
      ⛔ ``AUTO_PURCHASE_AFTER_TOPUP_ENABLED`` на боевом стоит **true** в ``system_settings``
      (в коде дефолт False, в ``.env`` ключа нет — проверено 30.08.2026), то есть это ЖИВОЙ путь,
      а не гипотеза;
    · включён автоплатёж — монитор спишет сам, и «сами не спишутся» будет прямой ложью.
    ...
    (`app/utils/funnel_state.py:102`). ⚠️ Совпадение НЕ побуквенное: меню сравнивает
    ОКРУГЛЁННЫЕ вниз дни, мы — точный срок, поэтому на промежутке 3–4 дня меню уже показывает
    «Продлить», а сообщение ещё молчит. Расхождение не вредит и названо, чтобы следующий не
    принял его за поломку.

    ``limited`` (исчерпан трафик) считается живой платной подпиской — так же, как в
    ...
    ⚠️ НАЗВАННАЯ ГРАНИЦА: у человека с сохранённой корзиной БЕЗ свежей метки на клавиатуре две
    кнопки покупки, и «кнопкой ниже» подписаны обе. Верная — вторая («Вернуться к оформлению»).
    Не лечим: обе ведут к покупке, деньгам ничего не грозит, а разведение кнопок — работа
    соседнего пункта.

    ⚠️ ПРОБЕЛ, НАЗВАННЫЙ ЧЕСТНО: сторожа на СОБРАННОЕ сообщение нет, есть только на эту функцию.
    Стенд для него существует (`tests/services/test_payment_service_webhooks.py:1120` — там ровно
    так проверяют текст у pal24), то есть прежняя отговорка «дороже пункта этапа» была
    преувеличением. Не написан сознательно, покрыт живым проходом.
    """
    try:
        from app.services.user_cart_service import user_cart_service
        ...
        if user_id is None:
            return None
        # ⛔ Забор по МЕТКЕ НАМЕРЕНИЯ, а не по наличию корзины, и разница здесь решающая.
        # Автопокупку запускает именно она (`subscription_auto_purchase_service.py:3339`), живёт
        # она 30 минут, а сама корзина — час. Гасили бы по корзине — заглушили бы ровно того,
        # кому фраза нужнее всех: человека, чья метка протухла, то есть автопокупка НЕ придёт,
        # и без нашей строки он не узнает об этом ничего.
        if await user_cart_service.has_topup_intent(user_id):
            return None
        ...
        for row in rows:
            if row.autopay_enabled and not row.is_trial:
                return None
            ...
            # Молчим у того, кому оформлять сейчас нечего: платная подписка с запасом больше
            # порога. Пробная сюда не попадает намеренно — ему как раз покупать.
            if (
                not row.is_trial
                and row.status in ('active', 'limited')
                and end_date > now + timedelta(days=threshold_days)
            ):
                return None

...
PERMISSION_REGISTRY: dict[str, list[str]] = {
    ...
    # 🔴 Больше кодов здесь не заводить «про запас»: пресеты раздают права точечно,
    # но объявленный код сразу появляется галочкой в редакторе ролей, а первый же
    # эндпоинт под ним окажется разрешённым задним числом.
    'auto_messages': ['read', 'edit'],
}

...
def _checkout_return_url(checkout_public_id: str, *, failed: bool = False) -> str | None:
    ...
    # `create_platega_attempt` пропускают только НЕ прямой.
    # 🔴 И ХОЗЯИНА У ЭТОЙ МИНЫ БОЛЬШЕ НЕТ. Прежняя редакция этого комментария назначала её
    # этапу Б-3 — неверно: оживление `legacy_deposit` владелец ОТКЛОНИЛ 24.08.2026 (решение
    # «дорога только кабинет», файл состояния). Значит путь не разбудит никто из
    # запланированного, и мина остаётся ждать того, кто когда-нибудь вернётся к этому режиму.
...
def _direct_checkout_return_url(checkout_public_id: str, *, failed: bool = False) -> str | None:
    ...
    """
    # 🔴 ЭТАП 2б (28.08.2026): В ТЕЛЕГРАМ УВОДИМ И ОТКАЗ ТОЖЕ. Прежний запрет снят вместе с
    # причиной, ради которой стоял, — это ботовая половина мины AQ.
    # ⛔ Прежний комментарий здесь гласил «`payment=failed` в кабинете сегодня не читает никто»
    # и «экран соврёт „Оплата учтена“». ОБА утверждения устарели, проверено по коду кабинета:
    # `DeviceFirstConfigurator.tsx:159-176` читает метку, показывает `providerDeclinedNotice`
    # («Платёжная система сообщила, что оплата не прошла») и сразу снимает её с адреса, чтобы
    # она не пережила перезагрузку. У кабинетной половины есть свой сторож
    # (`DeviceFirstConfigurator.interaction.test.tsx`, «мина AQ»). То есть экран научился
    # говорить, и удерживать человека на форме входа больше не за чем.
    # Замер на боевом 28.08.2026: 23 заказа из 32 отменённых закончились
    # `provider_terminal:canceled` — это самая частая причина обрыва, и каждый такой человек
    # упирался в форму входа вместо своего заказа.
    #
    # ⚠️ НЕ ПРЕУВЕЛИЧИВАТЬ, ЧТО ИМЕННО ОН ПРОЧТЁТ (поправлено линзой UX в тот же день).
    # Первая редакция этого комментария обещала, что вернувшийся увидит плашку «Оплата не
    # прошла». Для главной когорты это НЕ так: к моменту возврата заказ обычно уже закрыт
    # вебхуком, `ui_state` становится терминальным, и человек читает «Предыдущий счёт закрыт.
    # Не платите по старой ссылке» с кнопкой «Начать новый расчёт». Плашка отказа живёт в
    # ветке `awaiting_payment`, то есть достаётся тем, к кому вебхук ещё не успел.
    # Экран честен в обоих случаях, и правка полезна в обоих — но следующий этап нельзя
    # планировать на вере, что отказавшие читают именно плашку.
    #
    # 🔴 И ЕЩЁ ОДНО СЛЕДСТВИЕ, закрытое отдельным PR в кабинете: эта правка ВПЕРВЫЕ доводит
    # метку `payment=failed` до авторизованного экрана. Метка — слово провайдера, сказанное
    # редиректом, а провайдер уводит на `failedUrl` и когда платёж ещё жив. Экран покупки не
    # сверял её с сервером вовсе (сосед `TopUpResult.tsx:479-489` — сверяет). Забор поставлен
    # в кабинете: баннер отказа показывается только пока сервер ещё отдаёт адрес оплаты.
    #

...
def serialize_checkout(
    ...
    *,
    ...
    # тарифа): срок идёт от оплаты. Прибавлять его к концу пробного — обещать дни, которых не будет (до 3 дней).
    # Условие — то же, что у выдачи: флаг `is_trial` снимка, другой тариф и запрет переноса. База — день заказа,
    # не «сейчас»: заказ опрашивается, и дата не должна ползти. Пробный на том же тарифе продлевается с остатком.
    if (
    ...
    *,
    ...
    if checkout.funding_mode == 'wallet':
        ...
        if receipt is None:
            receipt = Transaction(
                ...
                # же помощником, что и весь проект: 180 дней он назовёт «6 месяцев».
                # ⛔ ЧИСЛА УСТРОЙСТВ ЗДЕСЬ НЕТ, И ЭТО НЕ ЗАБЫВЧИВОСТЬ. Отчёты о продажах
                # отличают докупку устройств от продажи подписки ЕДИНСТВЕННЫМ способом —
                # по слову «устройств» в описании (`crud/transaction.py`, DEVICE_ADDON_PATTERNS
                # = '%устройств%'). Любая форма слова содержит этот корень, поэтому упоминание
                # устройств увело бы КАЖДУЮ прямую продажу картой в статистику допов.
                # Поймано вопросом владельца «а это безопасно?», сторож рядом.
                # ⛔ Старую формулировку сюда не возвращать: на неё стоит растяжка в тестах.
                description=(
                    f'Платёж картой получен: подписка на {format_period_description(int(snapshot["period_days"]))}'
                ),
    ...
    if sale is None:
        sale = Transaction(
            ...
            # ⛔ Про отсутствие числа устройств — см. комментарий у прихода выше.
            description=(
...
def _client_ready_keyboard(user: User) -> InlineKeyboardMarkup:
    ...
    Второй повод, ради которого кнопка идёт ТЕМ ЖЕ пунктом, что и `openLink`.
    ⛔ ЗДЕСЬ СТОЯЛО «адрес возврата после оплаты — https-кабинет, а не `t.me`
    (`device_first_payment_service.py:1160`)». ОБЕ половины устарели, поправлено 28.08.2026:
    успех уводит диплинком в Телеграм с этапа В-1, отказ — с пункта 2б; а строка `:1160`
    лежит в спящем близнеце `_checkout_return_url`, тогда как живой
    `_direct_checkout_return_url` начинается на `:1204`. Кнопка от этого нужна не меньше:
    диплинк собирается только при известном `BOT_USERNAME`, иначе возврат по-прежнему падает
    на адрес сайта и в браузере приземляется на экран входа. Мини-приложение при этом
    остаётся живым за спиной у браузера, но человеку, который его закрыл, нужна дверь
    обратно — вот она.
...
async def refund_operator_review_checkout(
    ...
    *,
    ...
    # 🔴 РФ-4 (29.08.2026, прямое решение владельца). ЗДЕСЬ СТОЯЛО ОБРАТНОЕ:
    # `job.referral_status = 'done'` с обоснованием «комиссию с ВОЗВРАТА платить нельзя,
    # покупки не состоялось». Обоснование опровергнуто замером, а не мнением.
    #
    # ⛔ Во-первых, «возврат» — неточное слово: деньги никуда не вернулись, они легли клиенту
    # НА БАЛАНС и он ими воспользуется. Для клиента это неотличимо от пути А (поздний платёж
    # по закрытому счёту, `device_first_payment_service.py:2242`), а тот комиссию ПЛАТИТ.
    # Один и тот же исход давал разный ответ партнёру, и разницу определяло то, о чём ни
    # клиент, ни партнёр не знают: разбирал заказ живой оператор или заказ рассосался сам.
    #
    # ⛔ Во-вторых, «комиссия за состоявшуюся продажу» — не то правило, по которому живёт
    # проект. Замер на боевом 29.08.2026, все 61 выплата за историю: `referral_first_topup`
    # 30, `referral_commission_topup` 14, `referral_registration_pending` 17. За покупку
    # подписки не платилось НИ РАЗУ. Комиссия у нас возникает от денег НА БАЛАНСЕ — и путь Б
    # был единственным местом, выпадавшим из этого правила.
    #
    # ⇒ Двойной оплаты правка создать не может: покупка подписки комиссию не начисляет вовсе,
    # так что потраченный с баланса рубль второй раз партнёру ничего не принесёт.
    #
    # 🔴 НО ДЕРЖИТСЯ ЗАЩИТА НЕ НА ЭТОМ — поправлено линзой денег. Настоящий риск удвоения это
    # одни и те же провайдерские деньги, зачисленные дважды, и его держат ЗАБОРЫ выше в
    # `refundable_amount_kopeks`: отозванный платёж · уже созданная подписка
    # (`created_subscription_id`) · `already_credited` · удаляемый аккаунт. ⛔ Ослабишь любой
    # из них — например разрешишь возврат по попытке в статусе `credited` — и комиссия начнёт
    # удваиваться, а абзац выше будет утверждать, что это невозможно.
    #
    # ⛔ Признак берём из настройки, а НЕ жёстким `True`: `pay_referral=False` — аварийный
    # выключатель рефералки (РФ-1 п.1.1), и он обязан гасить в том числе этот путь. Зовём
    # ровно тем же способом, что и путь А, чтобы два пути нельзя было снова развести.
    # 🔴 РФ-4, дописано волной 2. ЭТОТ ВЫЗОВ — ЕДИНСТВЕННОЕ, что заводит обязательство перед
    # партнёром на этом пути. ⛔ Не приписывать снизу `job.referral_status = 'done'`: скептик
    # волны 2 показал экспериментом, что такой частичный откат гасит комиссию заново. Сторож
    # против него теперь есть (`test_..._pays_the_partner_like_any_other_top_up` проверяет и
    # аргумент, и итоговое состояние работы), но раньше оба теста это переживали.
    #
    # ⛔ И ЕЩЁ ОДНО, чего НЕ надо приписывать сюда «для симметрии с путём А»: явную побудку
    # воркера `process_device_first_deposit_outbox`. Соседние три пути её зовут, этот — нет,
    # и это осознанно оставлено: фоновое подметание забирает работу максимум за 60 секунд
    # (`device_first_recovery_service.py`), а лишний вызов на денежном пути — новый код там,
    # где вся ценность этапа в ОДНОЙ строке. Расхождение записано в план, а не замолчано.
    await ensure_deposit_outbox(
        db,

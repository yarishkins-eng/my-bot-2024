    ...
    *,
    ...
    if notification_type == ENTITLEMENT_DRIFT_NOTIFICATION_TYPE:
        ...
        advice = (
            ...
            'перенести отдельно, сам он туда не переедет. Готовой кнопки для этого сегодня нет — '
            'она появится вместе с переездом. ⛔ Не делайте это «Сбросить подписку»: она удалит '
            'оплаченную подписку целиком.'
        )
    else:

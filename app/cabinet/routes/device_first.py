...
async def _serialize_cabinet_checkout(
    ...
    *,
    ...
    # 🔴 Мина F добавила ВТОРОЙ такой экран. Брошенная корзина больше не висит в разборе, она
    # закрывается сама (`device_first_payment_service.py:720-736`) — а предупреждение «старая
    # ссылка ещё принимает деньги» обязано переехать вместе с ней, иначе человек, которому мы
    # только что вернули право купить, оплатит прежний счёт и получит баланс вместо подписки.
    # Спрашиваем те же факты, а не `terminal_reason` на фронте: при поздней оплате причина
    # заказа остаётся прежней, и ветка «по причине» сказала бы «денег не было», когда они есть.
    if payload.get('ui_state') in {'operator_review', 'cancelled'}:

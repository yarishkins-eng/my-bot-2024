...
- After the recovered old image, rollback source, deploy state, and keyed
  audit are all durable, the protected recovery workflow atomically changes
  the version 2 journal phase to `recovered`. It never calls the failed
  candidate `completed`. A crash before that last write leaves `prepared` and
  is completed idempotently by the next protected recovery retry.
...
- Candidate build and Alembic-head inspection use the clean target archive
  ...
  journal-bound state.
- The ordinary deploy workflow strictly validates both historical version 1
  and current version 2 journals, refuses an unresolved `prepared` migration
  journal, and requires current server source/image/schema to equal the
  durable normal or recovered deploy state before it evaluates any newer
  target. A later push therefore cannot cross or overwrite a protected
  migration recovery boundary. `recovered` is accepted only with exact
  agreement between journal identities, compatibility decision, live
  `mode=recovery` state, and the target-keyed audit; it remains the baseline
  for a later protected migration retry. A completed historical version 1
  journal is accepted with normal deploy state only when its migration target
  is in the current source ancestry and its target schema is still live.
...
- interruption after audit and before the atomic `recovered` journal phase;
...
Local pre-commit evidence: recovery/classifier/deploy-state matrix `55 passed`,
